import $ from 'cash-dom';
  ...
  M.textareaAutoResize = function($textarea) {
    ...
    if (!$textarea.result('original-height')) {
      $textarea.result('original-height', $textarea.height());
    }
     ...
     */
    if ($textarea.result('original-height') <= hiddenDiv.innerHeight()) {
      $textarea.css('height', hiddenDiv.innerHeight() + 'px');
    } else if ($textarea[0].value.length < $textarea.result('previous-length')) {
      /**
       * In case the new height is less than original height, it
       * means the textarea has less text than before
       * So we set the height to the original one
       */
      $textarea.css('height', $textarea.result('original-height') + 'px');
    }
    $textarea.result('previous-length', $textarea[0].value.length);
  };
  ...
  $(document).ready(function() {
    ...
    $(text_area_selector).each(function() {
       ...
       */
      if($textarea && $textarea.result) {
        $textarea.result('original-height', $textarea.height());
        $textarea.result('previous-length', this.value.length);
